export const NodeHelpers = {
  ...
  isNode(node) {
    return (
      !this.isProject(node) &&
      (node.category_id == "" || node.category_id == null)
    );
  },
  isTask(node) {
  ...
  areParentAndChild(node1, node2) {
    ...
    if (Object.keys(node1).length == 0 || Object.keys(node2).length == 0)
      return false;
    return (
      node1.ancestry == node2.id.toString() ||
      node2.ancestry == node1.id.toString()
    );
  }
};
...
export const TaskHelpers = {
  ...
  assignees() {
    if (
      this.inspected_node.hasOwnProperty("assignees") &&
      this.inspected_node.assignees instanceof Array &&
      this.members instanceof Array
    ) {
